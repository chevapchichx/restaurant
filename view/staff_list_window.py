from PyQt6.QtWidgets import (
    ...
    QWidget, QLabel, QTableWidget, QTableWidgetItem, QHeaderView, QMessageBox
)
...
class StaffListWindow(QWidget):
    ...
    def ui_staffs_info_window(self):
        ...
        self.add_staff_button.clicked.connect(
            lambda: open_add_staff_window(self))
        ...
        self.staffs_table.setColumnWidth(0, 220)
        self.staffs_table.setColumnWidth(1, 135)
        self.staffs_table.setColumnWidth(2, 125)
        self.staffs_table.setColumnWidth(3, 160)
        self.staffs_table.setHorizontalHeaderLabels(
            ["ФИО", "Должность", "Роль", "Телефон"])
        self.staffs_table.setSelectionBehavior(
            QTableWidget.SelectionBehavior.SelectRows)
        self.staffs_table.setEditTriggers(
            QTableWidget.EditTrigger.NoEditTriggers)

        self.staffs = sorted(UserService().get_staffs_info(),
        ...
        for i, staff in enumerate(self.staffs):
            item_full_name = QTableWidgetItem(
                f"{staff.last_name} {staff.first_name} {staff.middle_name}")
        ...
        self.staffs_table.cellDoubleClicked.connect(lambda: open_staff_info_window(
            self, self.staffs[self.staffs_table.currentRow()]))
