...
class Database_Service():
        ...
        try:
            with self.__engine.connect() as conn:
                if role == User_Role.ADMIN:
                    query = text("""
                        SELECT o.id, o.order_num, s.id, s.role, s.job, s.last_name, s.first_name, s.middle_name, o.id_table, o.order_date, o.order_time, o.order_status
                        FROM orders o
    ...
    def get_order_for_edit_db(self, id_order):
        try:
            with self.__engine.connect() as conn:
                query = text("""
                    SELECT o.id, o.order_num, s.id, o.id_table, o.order_date, o.order_time, o.order_status
                        FROM orders o
                        JOIN staff s ON o.id_worker = s.id
                    WHERE o.id = :id_order
                    """)
    ...
    def get_dishes_from_order_db(self, id_order):
        try:
            with self.__engine.connect() as conn:
                query = text("""
                    SELECT m.id, m.dish, m.price, m.weight, m.photo, m.id_menu_category, o_m.dish_status, o_m.amount
                    FROM menu m, orders o, orders_menu o_m
                    WHERE o.id = o_m.id_order AND m.id = o_m.id_dish AND o.id = :id_order
                    """)
